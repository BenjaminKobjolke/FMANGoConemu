...
class Logger:
    def __init__(self):
        self.log_file = os.path.join(os.environ.get('TEMP', 'C:\\Temp'), 'fman_conemu_debug.log')
...
def create_batch_file(path, conemu_path, log_file):
...
class MapNetworkDrive(DirectoryPaneCommand):
    def __call__(self):
        ...
        logger = Logger()
        log_file = logger.get_log_file_path()
        logger.log("=== New MapNetworkDrive execution ===")
...
__all__ = ['GoConemu', 'MapNetworkDrive']
